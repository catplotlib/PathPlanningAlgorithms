...
class AStarNode(RosNode):
    ...
    def initialize_start_and_goal(self):
        ...
        initial_state_range = [(-4.5, -3.5), (4.5, 3.5)]  # Example range

        # Generate a random start position within the initial state range
        start_x = random.uniform(initial_state_range[0][0], initial_state_range[0][1])
        start_y = random.uniform(initial_state_range[1][0], initial_state_range[1][1])
        ...
        # Initialize goal (top right corner)
    ...
    def run_astar(self):
        if self.costmap and self.start and self.goal:
            ...
            path = astar(start_index, goal_index, self.costmap.data, self.costmap.info.width, self.costmap.info.height)
            end_time = time.time()
...
def astar(start, goal, costmap, width, height):
    open_list = []
    closed_list = []
    ...
    while open_list:
        current_node = min(open_list, key=lambda node: node.cost + heuristic(node, goal_node))
